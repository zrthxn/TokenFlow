...
class TokenFlow(nn.Module):
    def __init__(self, config):
        ...
        self.scheduler.timesteps = self.scheduler.timesteps[
            int(1 - config["start"] * len(self.scheduler.timesteps)) :
        ]
    ...
    def get_latents_path(self):
        ...
        latents_path = [
            x for x in glob.glob(f"{latents_path}/*/*") if not x.startswith(".")
        ]
        n_frames = [
            int(
                [x for x in latents_path[i].split("/") if "nframes" in x][0].split("_")[
                    1
                ]
            )
    ...
    def get_data(self):
        # load frames
        frames = [
            Image.open(self.keyframes_path[idx]).convert("RGB")
            for idx in range(self.config["n_frames"])
        ]
    ...
    def batched_denoise_step(self, x, t, indices):
        ...
        pivotal_idx = torch.randint(batch_size, (len(x) // batch_size,)) + torch.arange(
            0, len(x), batch_size
        )  # [int(x) for x in torch.tensor((range(batch_size // 2, len(x) + batch_size // 2, batch_size)))]
    ...
    def edit_video(self):
        ...
        noise = (
            self.eps
            if config["use_ddim_noise"]
            else torch.randn_like(self.eps[[0]]).repeat(self.eps.shape[0])
        )
        noisy_latents = self.scheduler.add_noise(
            self.latents, noise, self.scheduler.timesteps[0]
        )
        edited_frames = self.sample_loop(
            noisy_latents, torch.arange(self.config["n_frames"])
        )
        save_video(
            edited_frames, f'{self.config["output_path"]}/tokenflow_SDEdit_fps_10.mp4'
        )
        save_video(
            edited_frames,
            f'{self.config["output_path"]}/tokenflow_SDEdit_fps_20.mp4',
            fps=20,
        )
        save_video(
            edited_frames,
            f'{self.config["output_path"]}/tokenflow_SDEdit_fps_30.mp4',
            fps=30,
        )
        print("Done!")
    ...
    def per_frame_sde(self):
        ...
        edited_frames = self.vanilla_sample_loop(
            noisy_latents, torch.arange(self.config["n_frames"])
        )
        save_video(edited_frames, f'{self.config["output_path"]}/vanilla_sde.mp4')
        save_video(
            edited_frames, f'{self.config["output_path"]}/vanilla_sde_fps20.mp4', fps=20
        )
        save_video(
            edited_frames, f'{self.config["output_path"]}/vanilla_sde_fps30.mp4', fps=30
        )
        print("Done!")
    ...
    def batch_vanilla_denoise_step(self, x, t, text_embed_input):
        ...
        for b in range(0, len(x), self.config["batch_size"]):
            denoised.append(
                self.vanilla_denoise(
                    x[b : b + self.config["batch_size"]], t, text_embed_input
                )
            )
    ...
    def vanilla_sample_loop(self, x, indices):
        ...
        text_embed_input = torch.cat(
            [torch.repeat_interleave(self.text_embeds, config["batch_size"], dim=0)]
        )
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="Sampling")):
        ...
        for i in range(len(decoded_latents)):
            T.ToPILImage()(decoded_latents[i]).save(
                f'{self.config["output_path"]}/img_ode_vanilla_sde/%05d.png' % i
            )
